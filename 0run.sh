...
model_type=yolox
job_name=yolox_s_coco
config=configs/${model_type}/${job_name}.yml
log_dir=log_dir/${job_name}

# 1. training
#CUDA_VISIBLE_DEVICES=0 python3.7 tools/train.py -c ${config}
#python3.7 -m paddle.distributed.launch --log_dir=${log_dir} --gpus 0,1,2,3,4,5,6,7 tools/train.py -c ${config} --fp16 --eval  # &> ${job_name}.log &

# 2. eval
#CUDA_VISIBLE_DEVICES=0 python3.7 tools/eval.py -c ${config} -o weights=https://paddledet.bj.bcebos.com/models/${job_name}.pdparams
#CUDA_VISIBLE_DEVICES=0 python3.7 tools/eval.py -c ${config} -o weights=../${job_name}.pdparams
...
#CUDA_VISIBLE_DEVICES=0 python3.7 tools/infer.py -c ${config} -o weights=../${job_name}.pdparams --infer_img=demo/000000014439.jpg
...
#CUDA_VISIBLE_DEVICES=0 python3.7 tools/export_model.py -c ${config} -o weights=../${job_name}.pdparams
