...
class Trainer_DenseTeacher(Trainer):
        ...
        for epoch_id in range(self.start_epoch, self.cfg.epoch):
            ...
            for step_id in range(len(self.loader)):
                ...
                if concat_sup_data:
                    ...
                    loss_dict_sup = self.model._layers(data_sup_s)
                else:
                    loss_dict_sup_w = self.model._layers(data_sup_w)
                    loss_dict_sup = self.model._layers(data_sup_s)
                ...
                if curr_iter > st_iter:
                    ...
                    student_preds = self.model._layers(data_unsup_s)

                    with paddle.no_grad():
                        data_unsup_w['is_teacher'] = True
                        teacher_preds = self.ema.model._layers(data_unsup_w)
    ...
    def get_distill_loss(self, head_outs, teacher_head_outs, ratio=0.1):
        ...
        for i in range(len(cls_thr)):
            cls_thr[i]=(cls_thr[i]+cls_thr_ig[i])/2
