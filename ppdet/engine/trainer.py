...
from ppdet.data.source.dataset import SemiDataset
import ppdet.utils.stats as stats
from ppdet.utils import profiler

from .callbacks import Callback, ComposeCallback, LogPrinter, SemiLogPrinter, Checkpointer, SemiCheckpointer, WiferFaceEval, VisualDLWriter, \
                        SniperProposalsGenerator
from ppdet.ssod.labelmatch_callbacks import LabelMatchCallback
from .export_utils import _dump_infer_config, _prune_input_spec

from ppdet.utils.logger import setup_logger
logger = setup_logger('ppdet.engine')

__all__ = ['Trainer', 'Semi_Trainer']
...
class Semi_Trainer(object):
    def __init__(self, cfg, mode='train'):
        ...
        sup_dataset = self.cfg['{}Dataset_sup'.format(capital_mode)] = create(
            '{}Dataset_sup'.format(capital_mode))()
        unsup_dataset = self.cfg['{}Dataset_unsup'.format(capital_mode)] = create(
            '{}Dataset_unsup'.format(capital_mode))()

        self.dataset = SemiDataset(sup_dataset, unsup_dataset)

        if self.mode == 'train':
                self.loader = create('Semi_{}Reader'.format(capital_mode))(
                    self.dataset, cfg.worker_num)
        # self.model.CLASSES = list(self.loader.dataset.sup.cname2cid.keys())
        # build model
        if 'model' not in self.cfg:
            self.model = create(self.cfg.ssod, classes=list(self.loader.dataset.sup.cname2cid.keys()), save_dir=self.cfg.save_dir)
        ...
        if self.model.debug:
            self.teacher_parameters_state = {}
            self.student_parameters_state = {}
        # self.teacher_parameters_state = {}
        # self.student_parameters_state = {}
        ...
        if self.use_ema:
            ema_decay = self.cfg.get('ema_decay', 0.999)
            ema_decay_type = self.cfg.get('ema_decay_type', 'ssl')
            self.ema = MeanTeacher(
                self.model,
                decay=ema_decay,
        ...
        if self.mode == 'train':
            ...
            self.optimizer = create('OptimizerBuilder')(self.lr, self.model.student)
    ...
    def train(self, validate=False):
        ...
        self.status.update({
            ...
            'step_id': 0,
            'steps_per_epoch': int(len(self.loader) / self.loader.batch_size)
        })
        ...
        for epoch_id in range(self.start_epoch, self.cfg.epoch):
            ...
            if self._nranks > 1:
                ...
                model.student.train()
            iter_tic = time.time()
            print("self.cfg.eval_interval:", self.cfg.eval_interval)
            ...
            for step_id, data in enumerate(self.loader):
                ...
                # model.train_cfg['rpn_pseudo_threshold'] = max(0.5,
                #                         min((step_id + 1)/len(self.loader)/16 * model.train_cfg['rpn_pseudo_threshold'],
                #                         model.train_cfg['rpn_pseudo_threshold'])
                #                     )
                # model.train_cfg['cls_pseudo_threshold'] = max(0.5,
                #                         min((step_id + 1)/len(self.loader)/16 * model.train_cfg['cls_pseudo_threshold'],
                #                         model.train_cfg['cls_pseudo_threshold'])
                #                     )
    ...
    def check_gradient(self):
        if len(self.teacher_parameters_state.keys()) == 0:
            for k, v in self.model.teacher.state_dict().items():
                self.teacher_parameters_state[k] = v.numpy()
            for k, v in self.model.student.state_dict().items():
                self.student_parameters_state[k] = v.numpy()
        else:
            total_teacher_sum = 0
            total_student_sum = 0
            for k, v in self.model.teacher.state_dict().items():
                total_teacher_sum += np.abs(v.numpy() - self.teacher_parameters_state[k]).sum()
            for k, v in self.model.student.state_dict().items():
                if 'weight' not in k and 'bias' not in k:
                    continue 
                total_student_sum += np.abs(v.numpy() - self.student_parameters_state[k]).sum()
            logger.info("check teacher parameters change is: {:.6f}".format(
                total_teacher_sum))
            logger.info("check student parameters change is: {:.6f}".format(
                total_student_sum))
