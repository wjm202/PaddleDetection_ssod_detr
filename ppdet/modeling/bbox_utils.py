...
def bbox2delta(src_boxes, tgt_boxes, weights):
    """生成 bbox 与 gt 的修正量
    Args:
        src_boxes:      (Tensor):           需要转换的 bbox, 形状为 (N, ..., 4)
        tgt_boxes:      (Tensor):           Gt bboxes, 形状为 (N, ..., 4)
    Returns:
        Tensor: (dx, dy, dw, dh) 形式的偏移
    """
    src_w = src_boxes[:, 2] - src_boxes[:, 0] # box 的宽高
    src_h = src_boxes[:, 3] - src_boxes[:, 1]
    src_ctr_x = src_boxes[:, 0] + 0.5 * src_w # bbox 的中点坐标
    src_ctr_y = src_boxes[:, 1] + 0.5 * src_h

    tgt_w = tgt_boxes[:, 2] - tgt_boxes[:, 0] # ground truth 的宽高
    tgt_h = tgt_boxes[:, 3] - tgt_boxes[:, 1]
    tgt_ctr_x = tgt_boxes[:, 0] + 0.5 * tgt_w # ground truth 的中点坐标
...
def delta2bbox(deltas, boxes, weights):
    ...
    Args:
        boxes:          (Tensor):           需要转换的 bbox, 形状为 (N, 4)
    ...
    pred_w = paddle.exp(dw) * widths.unsqueeze(1) # 计算经过网络偏移后的 roi 的中心点和宽高
