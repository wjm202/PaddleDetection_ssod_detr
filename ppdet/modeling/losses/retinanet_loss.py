...
class RetinaNetLoss(nn.Layer):
    ...
    def label_anchors(self, anchors, gt):
        ...
        for i in range(len(batch_gt_box)):
            ...
            matches_idxs, match_labels = label_box(
                anchors, gt_boxes, self.positive_thresh, self.negative_thresh,
                self.allow_low_quality, -1)

            if len(gt_boxes) > 0:
                matched_boxes_i = paddle.gather(gt_boxes, matches_idxs)
                matched_classes_i = paddle.gather(gt_classes, matches_idxs)
                matched_classes_i = paddle.where(
                    match_labels == 0,
                    paddle.full_like(matched_classes_i, self.num_classes),
                    matched_classes_i)
                matched_classes_i = paddle.where(
                    match_labels == -1,
                    paddle.full_like(matched_classes_i, -1), matched_classes_i)
            else:
                matched_boxes_i = paddle.zeros_like(anchors)
                matched_classes_i = paddle.zeros_like(
                    matches_idxs) + self.num_classes
    ...
    def forward(self, anchors, preds, inputs):
        ...
        pos_mask = paddle.logical_and(gt_labels >= 0,
        ...
        gt_labels = F.one_hot(
            paddle.gather(gt_labels, valid_idx),
        ...
        gt_deltas_list = [
            bbox2delta(anchors, gt_boxes[i], self.weights)
        ...
        if self.smoothl1_loss_delta > 0:
            reg_loss = F.smooth_l1_loss(
                ...
                gt_deltas,
                reduction="sum",
                delta=self.smoothl1_loss_delta)
        else:
