...
class MeanTeacher(object):
    ...
    def update(self, model=None):
        if self.ema_decay_type == 'threshold':
            decay = min(self.decay, (1 + self.step) / (10 + self.step))
        elif self.ema_decay_type == 'ssl':
            decay = min(self.decay, 1 - (1 + self.warmup) / (1 + self.warmup + self.step))
        elif self.ema_decay_type == 'exponential':
            decay = self.decay * (1 - math.exp(-(self.step + 1) / 2000))
        else:
            decay = self.decay
    ...
    def apply(self):
        ...
        state_dict = dict()
        for k, v in self.state_dict.items():
            if self.ema_decay_type != 'exponential' and self.ema_decay_type != 'ssl':
                v = v / (1 - self._decay**self.step)
            v.stop_gradient = True
            state_dict[k] = v
        return state_dict
