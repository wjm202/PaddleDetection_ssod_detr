...
@register
@serializable
class Semi_COCODataSet(DetDataset):
    """
    Load dataset with COCO format.
    ...
    def __init__(self,
                 ...
                 allow_empty=False,
                 empty_ratio=1.):
        super(Semi_COCODataSet, self).__init__(dataset_dir, image_dir, anno_path,
                                          data_fields, sample_num)
        self.load_image_only = False
        self.load_semantic = False
        self.load_crowd = load_crowd
    ...
    def __getitem__(self, idx):
        ...
        if len(self.transform) == 1:
            # roidb['tag'] = 'sup'
            # print("[DEBUG sup]:", roidb['im_file'])
            roidb['tag'] = 1
            return self.transform[0](roidb)
        # unsup
        elif len(self.transform) == 2:
            # print("[DEBUG unsup]:", roidb['im_file'])
            tea_roidb = copy.deepcopy(roidb)
            # tea_roidb['tag'] = 'unsup_teacher'
            tea_roidb['tag'] = 2
            stu_roidb = copy.deepcopy(roidb)
            # stu_roidb['tag'] = 'unsup_student'
            stu_roidb['tag'] = 3
            return [self.transform[0](tea_roidb), self.transform[1](stu_roidb)]
