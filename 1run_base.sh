...
#weights=output/fcos_r50_fpn_multiscale_2x_coco/0.pdparams  #/paddle/weights/{job_name}.pdparams

# 1. training
#CUDA_VISIBLE_DEVICES=6 python3.7 tools/train.py -c ${config} # -r ${weights}
python3.7 -m paddle.distributed.launch --log_dir=${log_dir} --gpus 0,1,2,3,4,5,6,7 tools/train.py -c ${config} #-r output/fcos_r50_fpn_multiscale_2x_coco/3 #--eval
