...
weight_path=/pytorch/weights/${job_name}_paddle.pdparams

# 1. training
#CUDA_VISIBLE_DEVICES=6 python3.7 tools/train.py -c ${config} #-r ${weight_path}
python3.7 -m paddle.distributed.launch --log_dir=${log_dir} --gpus 0,1,2,3,4,5,6,7 tools/train.py -c ${config} --eval #-r ${weight_path}
# &> ${job_name}.log &
